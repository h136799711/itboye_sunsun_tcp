...
class Events
{
    ...
    public static function onWorkerStart($businessWorker)
    {
        ...
        //记录Worker启动信息
        self::log($businessWorker->id, 'adt WorkerStart ');
        //清空首登设备
        self::getTcpClientDal()->clearAll();
        //清空日志
//        (new \sunsun\adt\dal\AdtTcpLogDal(self::$db))->clearAll();
        $time_interval = 30;
    ...
    /**
     * 获取 tcp_client 日志
     * @return \sunsun\adt\dal\AdtTcpClientDal
     */
    public static function getTcpClientDal()
    {
        if (self::$tcpClientDal == null) {
            self::$tcpClientDal = new \sunsun\adt\dal\AdtTcpClientDal(self::$db);
        }
    ...
    /**
     * 获取设备dal类
     * @return \sunsun\adt\dal\AdtDeviceDal
     */
    public static function getClientDal()
    {
        if (self::$clientDal == null) {
            self::$clientDal = new \sunsun\adt\dal\AdtDeviceDal(self::$db);
        }
    ...
    private static function process($did, $clientId, $originData)
    {
        ...
        $resp = (new \sunsun\adt\action\AdtProcessAction())->process($did, $clientId, $jsonDecode);
        return $resp;
    }
     ...
     * @param $message
     * @param $pwd
     * @return bool|\sunsun\adt\resp\AdtLoginResp
     */
    private static function login($client_id, $message, &$pwd)
    {
        ...
        $req = \sunsun\adt\req\AdtReqFactory::create(\sunsun\adt\req\AdtReqType::Login, $data);
        $did = $req->getDid();
        if (empty($did)) {
        ...
        $resp = new \sunsun\adt\resp\AdtLoginResp();
    ...
    {
//        \sunsun\helper\LogHelper::log(self::$db,$client_id,$message,'adt'.$type);
        \sunsun\adt\helper\AdtTcpLogHelper::log(self::$db, $client_id, $message, 'adt' . $type);
    }
}
