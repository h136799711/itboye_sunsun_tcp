...
class Events
{
    ...
    public static function onWorkerStart($businessWorker)
    {
        ...
        //记录Worker启动信息
        self::log($businessWorker->id, 'aph300 WorkerStart ');
        //清空首登设备
...
//        (new \sunsun\aph300\dal\Aph300TcpLogDal(self::$db))->clearAll();
        ...
        \Workerman\Lib\Timer::add($time_interval, function () {
            ...
            foreach ($allSessions as $clientId => $session) {
                ...
                if ($nowTime - $lastActiveTime > self::$inactiveTimeInterval) {
                    $msg = "aph300 tcp server waiting for more than " . self::$inactiveTimeInterval . " seconds";
    ...
    /**
     * 获取 tcp_client 日志
     * @return \sunsun\aph300\dal\Aph300TcpClientDal
     */
    public static function getTcpClientDal()
    {
        if (self::$tcpClientDal == null) {
            self::$tcpClientDal = new \sunsun\aph300\dal\Aph300TcpClientDal(self::$db);
        }
    ...
    /**
     * 获取设备dal类
     * @return \sunsun\aph300\dal\Aph300DeviceDal
     */
    public static function getClientDal()
    {
        if (self::$clientDal == null) {
            self::$clientDal = new \sunsun\aph300\dal\Aph300DeviceDal(self::$db);
        }
    ...
    private static function process($did, $clientId, $originData)
    {
        ...
        $resp = (new \sunsun\aph300\action\Aph300ProcessAction())->process($did, $clientId, $jsonDecode);
        return $resp;
    }
     ...
     * @param $message
     * @param $pwd
     * @return bool|\sunsun\aph300\resp\Aph300LoginResp
     */
    private static function login($client_id, $message, &$pwd)
    {
        ...
        $req = \sunsun\aph300\req\Aph300ReqFactory::create(\sunsun\aph300\req\Aph300ReqType::Login, $data);
        $did = $req->getDid();
        if (empty($did)) {
        ...
        $resp = new \sunsun\aph300\resp\Aph300LoginResp();
    ...
    {
//        \sunsun\helper\LogHelper::log(self::$db,$client_id,$message,'aph300'.$type);
        \sunsun\aph300\helper\Aph300TcpLogHelper::log(self::$db, $client_id, $message, 'aph300' . $type);
    }
}
