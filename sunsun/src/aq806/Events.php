...
//        (new \sunsun\aq806\dal\Aq806TcpLogDal(self::$db))->clearAll();
    ...
    /**
     * 获取设备dal类
     * @return \sunsun\aq806\dal\Aq806DeviceDal
     */
    public static function getClientDal()
    {
        if (self::$clientDal == null) {
            self::$clientDal = new \sunsun\aq806\dal\Aq806DeviceDal(self::$db);
        }
    ...
    private static function process($did, $clientId, $originData)
    {
        ...
        $resp = (new \sunsun\aq806\action\Aq806ProcessAction())->process($did, $clientId, $jsonDecode);
        return $resp;
    }
     ...
     * @param $message
     * @param $pwd
     * @return bool|\sunsun\aq806\resp\Aq806LoginResp
     */
    private static function login($client_id, $message, &$pwd)
    {
        ...
        $req = \sunsun\aq806\req\Aq806ReqFactory::create(\sunsun\aq806\req\Aq806ReqType::Login, $data);
        $did = $req->getDid();
        if (empty($did)) {
        ...
        $resp = new \sunsun\aq806\resp\Aq806LoginResp();
