...
class Events
{
    ...
    public static function onWorkerStart($businessWorker)
    {
        ...
        //记录Worker启动信息
        self::log($businessWorker->id, 'filter_vat WorkerStart ');
        //清空首登设备
...
//        (new \sunsun\filter_vat\dal\FilterVatTcpLogDal(self::$db))->clearAll();
    ...
    /**
     * 获取 tcp_client 日志
     * @return \sunsun\filter_vat\dal\FilterVatTcpClientDal
     */
    public static function getTcpClientDal()
    {
        if (self::$tcpClientDal == null) {
            self::$tcpClientDal = new \sunsun\filter_vat\dal\FilterVatTcpClientDal(self::$db);
        }
    ...
    /**
     * 获取设备dal类
     * @return \sunsun\filter_vat\dal\FilterVatDeviceDal
     */
    public static function getClientDal()
    {
        if (self::$clientDal == null) {
            self::$clientDal = new \sunsun\filter_vat\dal\FilterVatDeviceDal(self::$db);
        }
    ...
    {
        StatisticClient::tick("FilterVat", 'message');
        // 统计的产生，接口调用的时间
        ...
        // 上报结果
        StatisticClient::report('FilterVat', 'message', $success, $code, $msg);
    }
    ...
    private static function process($did, $clientId, $originData)
    {
        ...
        $resp = (new \sunsun\filter_vat\action\FilterVatProcessAction())->process($did, $clientId, $jsonDecode);
        return $resp;
    }
     ...
     * @param $message
     * @param $pwd
     * @return bool|\sunsun\filter_vat\resp\FilterVatLoginResp
     */
    private static function login($client_id, $message, &$pwd)
    {
        ...
        $req = \sunsun\filter_vat\req\FilterVatReqFactory::create(\sunsun\filter_vat\req\FilterVatReqType::Login, $data);
        $did = $req->getDid();
        if (empty($did)) {
        ...
        $resp = new  \sunsun\filter_vat\resp\FilterVatLoginResp();
