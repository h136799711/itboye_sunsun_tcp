...
$task->onWorkerStart = function($task)
{
    ...
    $size = 2;//$input->getOption('size');
    $portClients = initClients();
    $time = microtime(true);
    $port = 8286;//$input->getOption('port');
    $times = 5*3600;//在线时间 秒
    $sockets = [];
    ...
    for($i=0;$i<$size;$i++){
        $one  = $clients[$i+$start];
        $tcpDevice = createTcp(new TcpDevice('tcp://101.37.37.167:'.$port,null, $one));
        array_push($sockets,$tcpDevice);
    }

    Timer::add(5,function() use ($sockets,$size) {
        for($i=0;$i<$size;$i++){
            $tcpDevice  = $sockets[$i];
